...
# defines number of rows and number of columns, placed here for easy adjustment
rows, columns = 6, 7
...
class miniMax:
    ...
    def boardScore(self, board, turn):
        ...
        arrayMid = [i for i in list((board[columns//2][:]))]
        midCount = arrayMid.count(players[turn])
        score += 10 + (midCount * 3)

        for x in range(rows):
            arrayRow = [i for i in list(board[x][:])]
            # print(arrayRow, end='    ')
        ...
        for y in range(columns):
            arrayCol = [i for i in list(board[:][y])]
            # print(arrayCol, end='   ')
